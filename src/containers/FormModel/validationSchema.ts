...
const {
  formField: { firstName, lastName, address1, email, city, zipcode, country, nameOnCard, cardNumber, expiryDate, cvv },
} = checkoutFormModel;
...
export default [
  Yup.object().shape({
    ...
    [address1.name]: Yup.string().required(`${email.requiredErrorMsg}`),
    [email.name]: Yup.string().required(`${email.requiredErrorMsg}`).matches(emailRegEx, email.invalidErrorMsg),
